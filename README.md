...
Caddy or nginx in front for TLS, or pass `-tls-cert/-tls-key`. Flags and env
...
`go build`, `go vet` and `go test ./...` are clean — 22 tests, though
`internal/store` and `internal/webfs` have none. Go 1.25, two direct deps
...
Fix both before this faces anything but your own tailnet.
